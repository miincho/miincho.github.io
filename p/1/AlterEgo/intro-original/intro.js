...
gsap.from('.room_titles_container', {
   autoAlpha: 0,
   y: -100, 
   duration: 1,
   scrollTrigger: {
...
const fearPopupContainer = document.querySelector('.fear_popups');
const fearPopups = document.querySelectorAll('.fear_popups > div');

// Use a single ScrollTrigger for the container
ScrollTrigger.create({
  trigger: fearPopupContainer,
  start: 'top 50%', // Adjust this value based on your needs
  end: '+=800',    // Adjust this value based on your needs
  scrub: true,
  // markers: true,
  onEnter: () => {
    // Trigger each individual popup with a randomized delay
    fearPopups.forEach(popup => {
      gsap.to(popup, {
        opacity: 1,
        duration: 0.5,
  ...
  onLeave: () => {
    // Hide popups when scrolling past the end point
    fearPopups.forEach(popup => {
      gsap.to(popup, {
        opacity: 0,
        duration: 0.3,
...
const letters = document.querySelectorAll('.pixel_room_number span');

// Function to add flickering effect to letters
const flickerLetters = () => {
  gsap.to(letters, {
    ...
    repeat: 1,
    onComplete: () => {
      // Update the letters with the next word
      updateText("02-");
    },
  });
};
...
const updateText = (newText) => {
  letters.forEach((letter, index) => {
    letter.textContent = newText[index];
  });
};

// Scroll trigger for the flickering effect
ScrollTrigger.create({
  trigger: '.room_titles_container',
  start: 'top 50%',
  end: '+=200',
