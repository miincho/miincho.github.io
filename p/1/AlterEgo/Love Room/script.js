...
/*white spheres*/
  ...
  function handleChoiceClickAndNavigate2(choiceId, nextPage) {
    // Store the clicked choice ID in local storage
    localStorage.setItem('clickedChoice2', choiceId);

    setTimeout(function () {
      // Navigate to the next page
      window.location.href = nextPage;
    }, 100); // Adjust the time delay if needed
  }
  ...
  document.getElementById('firstChoice').addEventListener('click', function() {
    handleChoiceClickAndNavigate2('firstChoice', '../03REGRET/var1/r3gr3t.html');
  });

  document.getElementById('secondChoice').addEventListener('click', function() {
    handleChoiceClickAndNavigate2('secondChoice', '../03REGRET/var1.5/r3gr3t.html');
  });

  document.getElementById('thirdChoice').addEventListener('click', function() {
    handleChoiceClickAndNavigate2('thirdChoice', '../03REGRET/var2/r3gret.html');
  });

  document.getElementById('fourthChoice').addEventListener('click', function() {
    handleChoiceClickAndNavigate2('fourthChoice', '../03REGRET/var2 .5/r3gret.html');
  });
...
const fourthCheckYes = document.getElementById('fourthCheckYes');
const clickSphere4 = document.getElementById('sphere4'); 

clickSphere4.addEventListener('click', () => {
    fourthCheck.style.transform = 'scale(1)';
});

fourthCheck.addEventListener('click', (event) => {
    if (event.target.classList.contains('no1')) {
        fourthCheck.style.display = 'none';
        fourthCheckYes.style.transform = 'scale(0)';
    }

    if (event.target.classList.contains('yes1')) {
        fourthCheck.style.display = 'none';
        fourthCheckYes.style.transform = 'scale(1)';
    }
});
