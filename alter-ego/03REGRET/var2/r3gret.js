...
function rotateCharacters(element) {
  ...
  const rotatedText = originalText.split('').map(char => {
    // Apply random padding between characters
    const padding = getRandomPadding();
    return char === ' ' ? char : `<span style="display: inline-block; padding-right: ${padding}px; transform: rotate(${getRandomRotation()}deg);">${char}</span>`;
  }).join('');
